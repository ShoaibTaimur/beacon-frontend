...
interface ActionMetadataItem {
  label: string;
  color: string;
  icon: JSX.Element;
}

const ACTION_METADATA: Record<string, ActionMetadataItem> = {
  ring: {
    label: 'Rung Device',
  ...
  locate: {
    ...
    icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25s-7.5-4.108-7.5-11.25a9 9 0 1115 0z" />
      </svg>
    ),
  },
  refresh: {
...
export default function HistoryPanel({ deviceId, deviceName, isOpen, onClose }: HistoryPanelProps) {
  ...
  const loadHistory = async (targetPage: number, replace = false) => {
    setLoading(true);
    setError('');
    try {
      const res = await getDeviceHistory(deviceId, targetPage);
      const newEvents = (res.data.logs || []).map((e: any) => ({
        ...e,
        id: e.id || e._id,
        actorEmail: e.actor?.email || 'Owner',
                  ...
                  <div key={event.id || event._id} className="relative">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 mt-1">
                        Triggered by <span className="text-slate-300 font-medium">{event.actorEmail}</span>
                      </p>
