...
export function AdminApkSettings({
  currentAdminEmail,
}: {
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(
        err.response?.data?.error || err.message || "Failed to save password",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleUpdateAdminEmail = async (e: React.FormEvent) => {
    ...
    try {
      await updateConfig("admin_config", {
  ...
  return (
    <div className="mb-10 rounded-3xl border border-cyan-500/10 bg-slate-950/40 p-6 backdrop-blur-xl relative overflow-hidden">
        ...
        <div className="w-10 h-10 rounded-xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center text-cyan-400">
          ...
          >
            <path
              ...
              d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.57-.598-3.751h-.152c-3.196 0-6.1-1.249-8.25-3.286zm0 13.036h.008v.008H12v-.008z"
            />
          </svg>
        </div>
        <div>
          ...
          <p className="text-xs text-slate-400">
            Manage security settings for Companion APK download
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <form onSubmit={handleUpdatePassword} className="space-y-4">
          <div>
            ...
            <p className="mt-1 text-[10px] text-slate-500 mb-2">
              Only users entering this password on the landing page will be
              allowed to download the APK.
            </p>
            {currentPassword && (
        ...
        <form
          onSubmit={handleUpdateAdminEmail}
          className="space-y-4 border-t md:border-t-0 md:border-l border-white/5 md:pl-8 pt-6 md:pt-0"
        >
          <div>
            <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">
              Admin Gmail Address
            ...
            <p className="mt-1 text-[10px] text-slate-500">
              Change this email to delegate admin access to another account. You
              can also edit this value directly in DB.
            </p>
          </div>
          <button
            type="submit"
